...
async function getJobApplicationsForUser(userParams) {
  const user = await findUser(userParams);
  return user.jobApplications;
}

async function addJobApplicationForUser(userParams, newJobAppParams) {
  const user = await findUser(userParams);
...
async function updateJobApplicationForUser(userParams, newJobAppParams) {
  const user = await findUser(userParams);
  user.jobApplications = replaceDoc(user.jobApplications, newJobAppParams);
  await user.save();
}

async function deleteJobApplicationForUser(userParams, jobAppParams) {
  const user = await findUser(userParams);
  user.jobApplications = removeDoc(user.jobApplications, jobAppParams);
  await user.save();
}

async function addContactForJobApp(userParams, jobAppParams, newContactParams) {
  const user = await findUser(userParams);
...
async function updateContactForJobApp(
  userParams,
  jobAppParams,
  newContactParams
) {
...
async function deleteContactForJobApp(userParams, jobAppParams, contactParams) {
  const user = await findUser(userParams);
  const jobApp = findDoc(user.jobApplications, jobAppParams);
